...
pub fn window_borders_elem(
    ...
    for (id, rect) in windows {
        ...
        let mut bottom = div();
        bottom.absolute().position(x, y + h - width).size(w, width).bg(color);
        let mut left = div();
        left.absolute().position(x, y).size(width, h).bg(color);
        let mut right = div();
        right.absolute().position(x + w - width, y).size(width, h).bg(color);
...
pub fn whichkey_elem(
    ...
    columns_div.flex_row().gap(COL_GAP).min_w_0().children(col_elems);
...
pub fn pane_elem(
    ...
    title_bar.absolute().position(0.0, 0.0).size(w as f32, bar_h).bg(theme.accent);
    let mut title_text = text(title);
    title_text
        .absolute()
...
pub fn chrome_scene(
    ...
    pieces.push(window_borders_elem(frame.geometry, frame.focus, scale, theme));
    ...
    if let Some(mb) = frame.minibuffer {
        pieces.push(minibuffer_elem(size.w, size.h, &mb.display(), mb.sigil_len(), theme));
    }
    ...
    for (id, rect) in frame.geometry {
        ...
        let severities = pane.line_severities(frame.lsp.diagnostics(pane.doc), first_line, lines.len());
        ...
        pane_el.absolute().position(rect.x as f32 * chrome_scale, rect.y as f32 * chrome_scale);
        pieces.push(pane_el);
    }
