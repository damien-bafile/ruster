...
fn every_bundled_query_compiles_against_its_grammar() {
    let cases: &[(&str, &str, tree_sitter::Language)] = &[
        ...
        (
            "scheme",
            include_str!("../queries/scheme/highlights.scm"),
...
fn javascript_highlights_keywords_strings_and_functions() {
    ...
    assert!(
        !e.highlight_line(0).is_empty(),
        "comment line is highlighted"
    );
    assert!(
        !e.highlight_line(1).is_empty(),
        "const/arrow line is highlighted"
    );
    assert!(!e.highlight_line(4).is_empty(), "class line is highlighted");
    assert!(
        distinct_colors(&e, 1) > 1,
        "more than one syntax group on the const line"
    );
}
...
fn typescript_highlights_types_and_interfaces() {
    ...
    assert!(
        !e.highlight_line(0).is_empty(),
        "interface declaration is highlighted"
    );
    assert!(
        !e.highlight_line(1).is_empty(),
        "property signature is highlighted"
    );
    assert!(
        !e.highlight_line(3).is_empty(),
        "typed function signature is highlighted"
    );
    assert!(
        distinct_colors(&e, 3) > 1,
        "more than one syntax group on the signature line"
    );
}
...
fn every_parseable_language_has_a_highlight_query() {
    for ext in [
        ...
        assert!(
            crate::language_for_ext(ext).is_some(),
            "{ext} has a grammar"
        );
