...
fn main() {
    ...
    println!("  cold parse        {:>7.2} ms", t.elapsed().as_secs_f64() * 1000.0);
    ...
    for _ in 0..20 {
        ...
        tr.edit(&InputEdit {
            start_byte: at, old_end_byte: at, new_end_byte: at + 1,
    ...
    println!("  incremental       {:>7.2} ms", t.elapsed().as_secs_f64() * 1000.0 / 20.0);

    let t = std::time::Instant::now();
    for _ in 0..5 { std::hint::black_box(p.parse(&edited, None).unwrap()); }
    println!("  full (no tree)    {:>7.2} ms", t.elapsed().as_secs_f64() * 1000.0 / 5.0);
    ...
    let t = std::time::Instant::now();
    for _ in 0..5 { std::hint::black_box(ruster_syntax::bench_bracket_depths(&edited)); }
    println!("  bracket depths    {:>7.2} ms", t.elapsed().as_secs_f64() * 1000.0 / 5.0);
