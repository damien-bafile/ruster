...
pub fn char_to_line(editor: &dyn EditorView, char_idx: usize) -> usize {
    ...
    for line in 0..editor.buffer().line_count() {
        if editor.buffer().line_start_char(line) <= char_idx {
            acc = line;
        } else {
            break;
        }
...
mod tests {
    ...
    fn wbe_word_motions() {
        ...
        run(
            "hello world",
            &[KeyEvent::Char('w'), KeyEvent::Char('b')],
            0,
        );
        run("hello world", &[KeyEvent::Char('e')], 4);
        run(
            "hello world",
            &[KeyEvent::Char('w'), KeyEvent::Char('e')],
            10,
        );
    }
    ...
    fn count_prefix() {
        run(
            "hello world",
            &[KeyEvent::Char('3'), KeyEvent::Char('l')],
            3,
        );
        run(
            "hello world",
            &[KeyEvent::Char('2'), KeyEvent::Char('w')],
            11,
        );
    }
