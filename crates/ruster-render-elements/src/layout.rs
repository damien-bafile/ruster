...
pub fn layout(area: PxRect, root: &Elem, measurer: &mut impl TextMeasurer) -> LayoutScene {
    ...
    let root_node = tree.new_with_children(root_style, &[]).expect("taffy root creation cannot fail");
    ...
    tree.compute_layout_with_measure(
        ...
        |_known, _available, _node, ctx, _style| match ctx {
            Some(NodeCtx {
                kind: NodeKind::Text { line, font_size, family },
                ..
            }) => {
    ...
    read_back(&scene_root, Point { x: area.x, y: area.y }, &tree, root_fg, &root_key, &mut scene);
    scene
}
...
fn build_children(
    ...
    push_children(tree, children, parent_key, parent.style.fg.or(parent_fg), &mut child_nodes);

    let ids: Vec<NodeId> = child_nodes.iter().map(|n| n.node).collect();
    tree.set_children(parent_mirror.node, &ids).expect("set_children cannot fail");
...
fn read_back(
    ...
    let layout = tree.layout(node.node).expect("layout for a node we just computed");
    let abs = Point {
    ...
    if node.style.bg.is_some() || node.style.border_width > 0.0 {
        ...
        scene.boxes.push(BoxNode {
            ...
            fill: node.style.bg.map(Into::into).unwrap_or((1.0, 1.0, 1.0, 1.0)),
    ...
    if let NodeKind::Text { line, font_size, family } = &node.ctx.kind {
...
mod tests {
    ...
    impl TextMeasurer for MockMeasurer {
        fn measure(&mut self, line: &StyledLine, font_size: f32, _family: FontFamily) -> (f32, f32) {
    ...
    fn statusline(w: f32, h: f32) -> Elem {
        ...
        let pad = 12.0;
        let mode = build(|m| {
            m.id("mode").absolute().position(0.0, 0.0).size(64.0, bar_h).bg(Color::Rgb(69, 71, 90));
        ...
        let bar = build(|b| {
            b.id("bar").absolute().position(0.0, h - bar_h).size(w, bar_h).bg(Color::Rgb(30, 30, 30));
    ...
    fn statusline_bar_mode_and_children_land_at_the_hardcoded_numbers() {
        ...
        let bar_h = 40.0;
        let pad = 12.0;
        let scene = layout_at(&statusline(w, h), PxRect { x: 0.0, y: 0.0, w, h });

        let bar = find_box(&scene, &ElementKey(vec!["bar".into()]));
        assert_eq!(bar.rect, PxRect { x: 0.0, y: h - bar_h, w, h: bar_h });

        let mode = find_box(&scene, &ElementKey(vec!["bar".into(), "mode".into()]));
        assert_eq!(mode.rect, PxRect { x: 0.0, y: h - bar_h, w: 64.0, h: bar_h });

        let bar_key = ElementKey(vec!["bar".into()]);
        let ys = h - bar_h + pad;
        let n = find_text(&scene, &ElementKey(vec!["bar".into(), "mode".into(), "n".into()]));
        assert_eq!(n.rect, PxRect { x: 24.0, y: ys, w: 8.0, h: 28.0 });
        let ws = find_text(&scene, &bar_key.child("ws"));
        assert_eq!(ws.rect, PxRect { x: 76.0, y: ys, w: 16.0, h: 28.0 });
        let ind = find_text(&scene, &bar_key.child("ind"));
        assert_eq!(ind.rect, PxRect { x: 112.0, y: ys, w: 8.0, h: 28.0 });
        let title = find_text(&scene, &bar_key.child("title"));
        assert_eq!(title.rect, PxRect { x: 140.0, y: ys, w: 40.0, h: 28.0 });
    }
    ...
    fn which_key_panel_is_sized_from_its_column_chunks() {
        ...
        let panel = build(|p| {
            p.id("panel").absolute().position(12.0, 12.0).flex_col().gap(8.0).padding(8.0);
        ...
        let scene = layout_at(&root, PxRect { x: 0.0, y: 0.0, w: 400.0, h: 300.0 });

        let panel = find_box(&scene, &ElementKey(vec!["panel".into()]));
        assert_eq!(panel.rect, PxRect { x: 12.0, y: 12.0, w: 128.0, h: 86.0 });

        let cols = find_box(&scene, &ElementKey(vec!["panel".into(), "cols".into()]));
        assert_eq!(cols.rect, PxRect { x: 20.0, y: 46.0, w: 112.0, h: 44.0 });

        let col1 = find_box(&scene, &ElementKey(vec!["panel".into(), "cols".into(), "col1".into()]));
        assert_eq!(
            col1.rect,
            PxRect { x: 20.0, y: 46.0, w: 72.0, h: 44.0 },
            "a column's width is the max over its rows of key_w + 8 + desc_w"
        );

        let r1 = find_box(&scene, &ElementKey(vec!["panel".into(), "cols".into(), "col1".into(), "r1".into()]));
        ...
        assert_eq!(r1.rect, PxRect { x: 20.0, y: 46.0, w: 72.0, h: 20.0 });

        let d2 = find_text(
            &scene,
            &ElementKey(vec!["panel".into(), "cols".into(), "col1".into(), "r2".into(), "d2".into()]),
        );
        assert_eq!(d2.rect, PxRect { x: 44.0, y: 70.0, w: 48.0, h: 20.0 });

        let k3 = find_text(
            &scene,
            &ElementKey(vec!["panel".into(), "cols".into(), "col2".into(), "r3".into(), "k3".into()]),
        );
        assert_eq!(k3.rect, PxRect { x: 100.0, y: 46.0, w: 8.0, h: 20.0 });
    }
    ...
    fn hover_panel_below_above_and_x_clamped() {
        ...
        let anchor = PxRect { x: 100.0, y: 200.0, w: 50.0, h: 30.0 };
        let clamp = |x: f32| x.min(300.0 - w);

        let make = |x: f32, y: f32| {
            let panel = build(|p| {
                p.id("panel").absolute().position(x, y).flex_col().size(w, h);
                p.bg(Color::Rgb(30, 30, 30));
                p.children(vec![
                    txt("line1", |t| {
                        t.id("l1").h(18.0);
        ...
        let scene = |root: &Elem| layout_at(root, PxRect { x: 0.0, y: 0.0, w: 300.0, h: 200.0 });
        ...
        assert_eq!(panel.rect, PxRect { x: 100.0, y: 230.0, w, h });
        assert_eq!(
            find_text(&below, &ElementKey(vec!["panel".into(), "l1".into()])).rect,
            PxRect { x: 100.0, y: 230.0, w, h: 18.0 }
        );
        assert_eq!(
            find_text(&below, &ElementKey(vec!["panel".into(), "l2".into()])).rect,
            PxRect { x: 100.0, y: 248.0, w, h: 18.0 }
        );
        ...
        assert_eq!(panel.rect, PxRect { x: 220.0, y: 148.0, w, h });
    }
    ...
    fn pane_body_runs_land_at_glyph_origins() {
        ...
        let body = build(|b| {
            b.id("body").absolute().position(body_x, body_y).size(w - body_x, h - body_y);
            b.bg(Color::Rgb(10, 10, 10)).padding_left(col * CELL_W).padding_top(0.0);
            b.children(vec![txt("fn main()", |t| {
                t.id("run").font_size(16.0).h(20.0);
            })]);
        });
        let titlebar = build(|t| {
            t.id("titlebar").absolute().position(0.0, 0.0).size(w, FRAME_BAR_H).bg(Color::Rgb(40, 40, 40));
        });
        let mut root = div();
        root.children(vec![titlebar, body]);
        let scene = layout_at(&root, PxRect { x: 0.0, y: 0.0, w, h });

        let titlebar = find_box(&scene, &ElementKey(vec!["titlebar".into()]));
        assert_eq!(titlebar.rect, PxRect { x: 0.0, y: 0.0, w, h: FRAME_BAR_H });

        let body = find_box(&scene, &ElementKey(vec!["body".into()]));
        assert_eq!(body.rect, PxRect { x: body_x, y: body_y, w: w - body_x, h: h - body_y });

        let run = find_text(&scene, &ElementKey(vec!["body".into(), "run".into()]));
        assert_eq!(run.rect, PxRect { x: body_x + col * CELL_W, y: body_y, w: 72.0, h: 20.0 });
    }
    ...
    fn absolute_positioning_does_not_disturb_child_geometry() {
        let content = |into: &mut Elem| {
            into.children(vec![build(|b| {
                b.id("bar").size(200.0, 40.0).bg(Color::Rgb(1, 2, 3)).padding_left(8.0);
        ...
        let flow_scene = layout_at(&flow_root, PxRect { x: 14.0, y: 34.0, w: 200.0, h: 40.0 });

        // ...vs absolute at (14, 34) inside an un-offset viewport.
        let mut abs_root = div();
        abs_root.children(vec![build(|w| {
            w.id("wrap").absolute().position(14.0, 34.0).size(200.0, 40.0);
            content(w);
        })]);
        let abs_scene = layout_at(&abs_root, PxRect { x: 0.0, y: 0.0, w: 200.0, h: 40.0 });

        let box_rects = |s: &LayoutScene| -> Vec<PxRect> { s.boxes.iter().map(|b| b.rect).collect() };
        assert_eq!(
            box_rects(&abs_scene), box_rects(&flow_scene),
            "absolute wrapper must not move children"
        );
        let text_rects = |s: &LayoutScene| -> Vec<(PxRect, String)> {
            s.texts.iter().map(|t| (t.rect, t.line.text.clone())).collect()
        };
        assert_eq!(
            text_rects(&abs_scene), text_rects(&flow_scene),
            "absolute wrapper must not move children"
        );
    }
    ...
    fn reordering_siblings_remaps_their_keys_at_layout_level() {
        ...
        let keys = |root: &Elem| -> Vec<ElementKey> {
            layout_at(root, PxRect { x: 0.0, y: 0.0, w: 100.0, h: 50.0 })
                .texts
                .into_iter()
                .map(|t| t.key)
                .collect()
