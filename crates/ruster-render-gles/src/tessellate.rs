...
use crate::geometry::{rect_verts, rounded_rect_verts, ChromeBatch, GlyphQuad};
...
mod tests {
    ...
    fn text_node(rect: PxRect, text: &str, fg: (f32, f32, f32, f32), key: ElementKey) -> TextNode {
        TextNode {
            rect,
            line: StyledLine {
                text: text.to_string(),
    ...
    fn rounded_box_matches_rounded_rect_verts_and_clamps_radius() {
        ...
        assert!(
            batch.verts.len() > 6,
            "a rounded rect is more than one quad"
        );
    ...
    fn text_emits_the_same_quads_as_the_old_text_in() {
        ...
        let reference =
            reference_text_quads("hi!", 16, rect.x, rect.y, fg, FontFamily::Mono, &mut atlas);
    ...
    fn glyph_keys_are_stable_across_runs() {
        ...
        assert_eq!(
            ...
            "the atlas caches, so quads repeat"
        );
    }
    ...
    fn measurer_reports_what_the_atlas_will_tessellate() {
        ...
        assert_eq!(
            height, 20.0,
            "font_size + 4.0, the cell_metrics height rule"
        );
    }
