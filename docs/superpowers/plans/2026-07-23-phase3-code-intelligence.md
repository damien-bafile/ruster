...
- [x] **Step 1:** Add a `Lang` discriminant (or reuse the extension) so `SyntaxEngine`
...
- [x] **Step 2:** Add `highlights.scm` for python, javascript, typescript, json, toml,
  yaml, c (vendored from nvim-treesitter or minimal hand-written); keep rust's textobjects.
- [x] **Step 3:** In `app.rs`, replace `syntax`/`syntax_buffer` with
...
- [x] **Step 4: Tests:** a Python source highlights with Python queries (not Rust); opening
  a second file of a different language highlights correctly; unsupported ext → plain text.
- [x] **Step 5:** `cargo test -p ruster-syntax -p ruster-tui`.
- [x] **Step 6:** Commit: `fix: per-language tree-sitter queries and per-buffer syntax`
...
- [x] **Step 1:** New crate with deps `lsp-types`, `serde`, `serde_json`. Add to workspace.
- [x] **Step 2:** `transport.rs`: read/write LSP frames (`Content-Length` headers). A reader
  thread parses frames off a `Read` and sends `ServerMessage { Response | Notification }`
  over an `mpsc::Sender`.
- [x] **Step 3:** `client.rs`: `LspClient::spawn(cmd, args, root)` starts the child with
  piped stdio and the reader thread. `request(method, params) -> id`, `notify(method,
  params)`, `poll() -> Vec<ServerMessage>`, `shutdown()`.
- [x] **Step 4: Tests:** frame round-trip (encode then decode yields the same JSON); a
  fake in-memory server (two pipes) exercises request → response id correlation and a
  notification, with no real process.
- [x] **Step 5:** `cargo test -p ruster-lsp`.
- [x] **Step 6:** Commit: `feat: ruster-lsp JSON-RPC transport and client`
...
- [x] **Step 1:** `registry.rs`: filetype → (command, args) defaults (rust-analyzer,
...
- [x] **Step 2:** On buffer open of a supported filetype: `didOpen` (uri, language, version 0,
  text). On edit: bump version, `didChange` (full text). On close/quit: `didClose` /
  `shutdown` + `exit`.
- [x] **Step 3:** In `App::render` (or a per-frame update), `poll()` every client and route
  messages: notifications to a handler, responses to pending-request dispatch (Task 4+).
- [x] **Step 4: Tests:** registry returns the right default command per filetype; version
  counter increments on edits; poll routes a fake notification. (No live server.)
- [x] **Step 5:** `cargo test -p ruster-lsp -p ruster-tui`.
- [x] **Step 6:** Commit: `feat: LSP server registry, lifecycle, and document sync`
...
- [x] **Step 1:** Handle `textDocument/publishDiagnostics`: store `Vec<Diagnostic>` keyed by
  buffer. Map LSP ranges to ruster line/col.
- [x] **Step 2:** Render: diagnostic sign in the gutter (`E`/`W`, colored) per affected line;
...
- [x] **Step 3:** `:diagnostics` / `Space c d` opens diagnostics in a Picker; Enter jumps.
- [x] **Step 4: Tests:** parse a `publishDiagnostics` payload into the store; gutter-sign
  computation marks the right lines; picker lists them.
- [x] **Step 5:** `cargo test -p ruster-tui`.
- [x] **Step 6:** Commit: `feat: LSP diagnostics with gutter signs and list`
...
- [x] **Step 1:** `K` / `Space c k` sends `textDocument/hover` at the cursor; on response,
  store the markdown/plaintext contents and show a floating bordered popup near the cursor.
- [x] **Step 2:** Render the popup in both backends (reuse the picker-style box); dismiss on
  the next key.
- [x] **Step 3: Tests:** hover response contents parse into popup lines; popup wraps/truncates.
- [x] **Step 4:** `cargo test -p ruster-tui`.
- [x] **Step 5:** Commit: `feat: LSP hover popup`
...
- [x] **Step 1:** `gd` / `Space c g` sends `textDocument/definition`; on response, open the
  target (reuse `open_path(path, Some((line, col)))`), handling `Location` / `LocationLink`
  and single-vs-array results.
- [x] **Step 2: Tests:** parse definition result variants into a (path, line, col).
- [x] **Step 3:** Commit: `feat: LSP go-to-definition`
...
- [x] **Step 1:** `gr` / `Space c r` sends `textDocument/references`; on response, populate a
  Picker of `file:line:col` locations (reuse the picker + `OpenLocation`).
- [x] **Step 2: Tests:** references array parses into picker items.
- [x] **Step 3:** Commit: `feat: LSP find references`
...
- [x] **Step 1:** `Space c n` prompts (mini-buffer) for a new name → `textDocument/rename` →
  apply the `WorkspaceEdit` (text edits across affected buffers, opening them as needed).
- [x] **Step 2:** `Space c f` / `:fmt` sends `textDocument/formatting`; apply returned edits
  to the buffer. Optional format-on-save behind a config flag.
- [x] **Step 3: Tests:** apply a `WorkspaceEdit`/edit list to a buffer produces expected text
  (pure edit-application helper, no server).
- [x] **Step 4:** Commit: `feat: LSP rename and formatting`
...
- [x] **Step 1:** `Space c o` sends `textDocument/documentSymbol`; render a hierarchical
  outline (a Special buffer like dired, or a side split); Enter jumps to the symbol.
- [x] **Step 2:** `Space c s` sends `workspace/symbol` with the typed query → Picker → jump.
- [x] **Step 3: Tests:** documentSymbol (both `DocumentSymbol` tree and `SymbolInformation`
  flat) parse into outline entries.
- [x] **Step 4:** Commit: `feat: LSP document outline and workspace symbol search`
...
- [x] **Step 1:** `snippets.rs`: parse snippet bodies with tabstops (`$1`,`$2`,`$0`,
...
- [x] **Step 2:** In `app.rs`, expand the trigger word before the cursor on a key (e.g. Tab
  in insert when a trigger matches); place the cursor at `$1`, cycle stops with Tab/Shift-Tab.
- [x] **Step 3: Tests:** parsing tabstops; expansion text + stop ranges; cycling order.
- [x] **Step 4:** Commit: `feat: snippet engine with tabstops`
...
- [x] **Step 1:** Lua LSP config: `ruster.lsp.servers[filetype] = { cmd, args }` to override
  the registry; `format_on_save` flag; snippet registration hook.
- [x] **Step 2:** Expose events/handlers (`ruster.on("LspAttach", …)`, diagnostics count for
  the statusline).
- [x] **Step 3:** Update `docs/lua-api.md` and `docs/config-reference.md` for every new
  setting, command, keybinding, and API.
- [x] **Step 4:** Commit: `feat: Lua LSP config and Phase 3 docs`
