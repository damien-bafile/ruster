...
| `whichkey_elem(output_w, output_h, view, theme, measurer)` measuring via measurer | The panel's width comes from taffy sizing the text leaves; but the **column chunking** (`per_col`, `columns`, `rows_drawn`, `w`/`h`) is pure arithmetic already done in `draw_whichkey` | `whichkey_elem` keeps that arithmetic, applies `.max_w(output_w - 24)`, and lets taffy size columns from measured text; parity test verifies equality. `Styled` has no `max_w`, so it was added to the elements crate (mirrors `min_w_0`, one line) — the only way to express the clamp without naming `taffy::geometry::Dimension` in the compositor |
| The which-key title's measured width | `draw_whichkey` sizes the panel from the **columns alone**; the title is drawn but never widens it. A fit-content panel would widen to a long title and diverge | The title text gets `.w(0.0)` so taffy gives it a plain pen position and the panel width tracks only the columns; glyphs still render (layout ignores the box width) |
| Parity test laying a widget out directly | `layout()` replaces the **root** element's geometry with the viewport, so `statusline_elem`/etc as the root lose their absolute position | The test's `new_batch` wraps the widget in a plain `div()` viewport root (the elements crate's own pattern), so the widget's absolute geometry is what `chrome_scene`'s `compose` would give it |
...
- [x] **TDD fail:** a parity test (below) fails because the builders do not exist.
- [x] `scene.rs` (imports `ruster_render_elements::{div, text, Elem, Styled, FontFamily, PxRect}`, render-gles `GlesTextMeasurer`, chrome helpers):
...
- [x] **Verify:** `cargo test -p ruster-compositor` green (old + new paths coexist); clippy clean.
