...
| A restored pane comes back with its file | quit with a file open in a pane, boot again | ⛔ **implemented, not yet seen end to end.** The session format now carries the path as the `pane` keyword's argument (`pane /src/main.rs`), a scratch pane stays a bare `pane`, and older files still parse. Round trip and both mutations are guarded. What could not be verified is the whole cycle: the nested compositor's event loop stalled with the host not presenting to its window, so it never processed the shutdown that writes the session. Needs one clean nested or DRM run |
| The event loop can stall and then ignore signals | (observed, not yet diagnosed) | ⚠️ a nested compositor whose host has stopped presenting stopped iterating, and then ignored both SIGINT and SIGTERM — the handler sets a flag the loop never gets back to reading. Harmless nested, since the window can be closed; on DRM it would mean a session killable only by its own quit binding. Worth understanding before the next hardware trip |
