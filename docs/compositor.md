...
| Screenshot works on DRM | press the screenshot bind on a hardware boot | ⛔ **it does not** — `backend/drm.rs` never reads `screenshot_pending`, so the action fires, logs nothing, writes nothing. The capture exists precisely because a DRM session has no other way to show what it drew, and it was only ever wired into the winit path. Found by mining a hardware log for a PNG that was never written |
