...
| The DRM capture is the right way up | look at one | ⚠️ **it was not, now fixed but unconfirmed.** The first capture ever taken came out vertically mirrored. `capture` flipped rows unconditionally because a direct GL framebuffer read is bottom-left first — but the DRM path reads back a texture `blit_frame_result` already wrote top-left, so the flip inverted it. The flip is now per-backend. Needs one more boot to confirm |
