...
| A restored pane comes back with its file | quit with a file open in a pane, boot again | ⛔ **it does not.** The session format records a pane and its title, not its path, so a restored pane is empty under its old name. The migration was half-written when the agent doing it stopped and was reverted rather than guessed at — a known gap, not a corrupt format |
