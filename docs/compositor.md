...
| Focus is visible on screen | tile two windows, move focus | ⛔ **written, not yet seen.** Every window had the same shape and the same chrome, so nothing said which one took the next keystroke. Borders are now drawn per window in `border_focused`/`border_unfocused`. Unit-tested for colour distinction (mutation-tested), containment within the window, and output scaling; the nested visual check is still owed because the graphical session was switched away from its VT at the time |
| Compositor uses the configured theme | set a theme in `config.lua`, boot the compositor | ⛔ **written, not yet seen.** It drew with `Theme::default()` while `ruster-lua` sat unimported in its Cargo.toml, so no built-in theme and no colour override reached compositor chrome at all. It now loads `config.lua` then `init.lua`, the same order as the editor |
