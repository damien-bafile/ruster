...
| Workspace layouts survive a restart | quit and reboot the compositor | ⛔ **written, unproven.** Trees are saved per workspace and restored next boot. A leaf is a live client, so it can only be relaunched when the command behind it is known — which is true only for processes the compositor started (`spawn_command` returns the pid, and a client's credentials give the pid back). Windows started from inside a terminal are saved with their title and no command, and are not relaunched: guessing a command from a title returns a session that is subtly wrong, which is worse than one that is visibly short. Needs a boot |
