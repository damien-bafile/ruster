...
| `:Git` `:git` | Git status: staged, unstaged and untracked files in foldable sections. `s` stages the file under the cursor, `u` unstages it, `c` commits, `d` shows the staged diff, `P` pushes, `F` pulls, `Enter` opens it, `Tab`/`z` folds a section, `r`/`g` refreshes, `q` closes |
...
| `:GitStaged` `:staged` | Show the staged diff. `u` unstages the hunk under the cursor, `r`/`g` refreshes, `q` closes |
